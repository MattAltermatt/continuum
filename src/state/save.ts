...
import { withoutOrphans } from '../engine/automation';
...
export function reconcile(state: GameState, book: Book): GameState {
  ...
  return {
    ...
    // A supply whose order the book no longer has leaves with it, as it would have on an x.
    queue: withoutOrphans(state.queue.filter((e) => row(e.actionId))),
