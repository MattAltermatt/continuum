...
const skill = { id: 'forage', name: 'Forage', icon: 'sprout' } as const;
...
describe('SkillCell', () => {
  it('shows the name, the multiplier from balance, and Lv on each line', () => {
    render(<SkillCell skill={skill} state={forage} running={false} />);
  ...
  it('shows the level-up mark and the time to the next core level only when running', () => {
    render(<SkillCell skill={skill} state={forage} running={true} />);
  ...
  it('marks the running cell as working, and names it for assistive tech', () => {
    const { container } = render(<SkillCell skill={skill} state={forage} running={true} />);
