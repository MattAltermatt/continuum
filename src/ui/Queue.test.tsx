...
describe('Queue', () => {
  ...
  it('says waiting only when nothing can run: not while something runnable is queued, and not while paused', () => {
    const mixed = enqueue(enqueue(newState(), scrub, 'forage'), scrub, 'cabin');
    const one = render(<Queue state={mixed} content={scrub} working={0} live={true} onRemove={noop} />);
    expect(screen.queryByText('waiting')).toBeNull();
    one.unmount();
    const stuck = enqueue(newState(), scrub, 'cabin');
    const two = render(<Queue state={stuck} content={scrub} working={-1} live={false} onRemove={noop} />);
    expect(screen.queryByText('waiting')).toBeNull();
    two.unmount();
    render(<Queue state={stuck} content={scrub} working={-1} live={true} onRemove={noop} />);
    expect(screen.getByText('waiting')).toBeInTheDocument();
  });
  it('shows no countdown on an entry while paused or dead', () => {
    render(<Queue state={enqueue(newState(), scrub, 'forage')} content={scrub} working={0} live={false} onRemove={noop} />);
  ...
  it('on a dead run the remove buttons say they are inert and do nothing, even on a costly entry', () => {
    const onRemove = vi.fn();
    const base = enqueue(enqueue(newState(), scrub, 'forage'), scrub, 'cabin');
    const s = { ...base, queue: [base.queue[0]!, { ...base.queue[1]!, costsConsumed: 5, progress: 50 }], dead: true };
    render(<Queue state={s} content={scrub} working={-1} live={false} dead={true} onRemove={onRemove} />);
  ...
  it('an armed remove goes back to x when the run dies', () => {
    ...
    const { rerender } = render(<Queue state={s} content={scrub} working={-1} live={true} onRemove={noop} />);
    act(() => { screen.getByRole('button', { name: 'remove Build a cabin' }).click(); });
    expect(screen.getByText('?')).toBeInTheDocument();
    rerender(<Queue state={{ ...s, dead: true }} content={scrub} working={-1} live={false} dead={true} onRemove={noop} />);
    expect(screen.queryByText('?')).toBeNull();
